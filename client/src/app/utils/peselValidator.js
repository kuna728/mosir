...
export default function peselValidator(pesel, {dateOfBirth, gender}) {
    // console.log("in peselValidator with args:")
    // console.log("    pesel=" + pesel)
    // console.log("    dateOfBirth=" + dateOfBirth)
    // console.log("    gender=" + gender)
    if(!(typeof pesel === 'string' &&  /^\d+$/.test(pesel) && pesel.length===11))
        return false;
    ...
    return validateDatePart(datePart) && datesEqual(datePart, dateOfBirth)
           && genderEqual(genderPart, gender) && checkSumEqual(pesel);
}
